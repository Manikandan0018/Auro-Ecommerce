  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden md:block w-1/2 relative overflow-hidden">
        <img src="https://images.unsplash.com/photo-1558769132-cb1aea458c5e?w=800&q=80" alt="" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0a0a0a] to-transparent" />
        <div className="absolute bottom-12 left-10">
          <div className="font-display text-7xl text-white leading-none">JOIN<br/>US.</div>
          <div className="text-gray-400 text-sm mt-3">Create your COLLUSION account</div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center px-8 md:px-16 lg:px-24">
        ...
        <p className="text-gray-500 text-sm mb-8">Join the COLLUSION community</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {[
            ...
            <div key={f.key}>
              ...
              <input type={f.type} value={form[f.key]} onChange={e => setForm({...form, [f.key]: e.target.value})}
                placeholder={f.placeholder} required className="w-full px-4 py-3 text-sm rounded-sm" />
            </div>
          ))}
          <div>
            ...
            <div className="relative">
              <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => setForm({...form, password: e.target.value})}
                placeholder="Min. 6 characters" required className="w-full px-4 py-3 text-sm rounded-sm pr-10" />
          ...
          <div>
            <label className="text-xs font-mono-custom uppercase tracking-widest text-gray-400 block mb-1.5">Confirm Password</label>
            <input type="password" value={form.confirm} onChange={e => setForm({...form, confirm: e.target.value})}
              placeholder="Repeat password" required className="w-full px-4 py-3 text-sm rounded-sm" />
          </div>
        ...
        <p className="text-center text-xs text-gray-500 mt-6 font-mono-custom">
          Have an account? <Link to="/login" className="text-[#C6F135] hover:underline">SIGN IN</Link>
        </p>
      </div>
    </div>
  );
