...
export default function AdminLayout() {
  const { currentUser, logout, getAdminNotifications } = useApp();

  const notifications = getAdminNotifications ? getAdminNotifications() : [];
    ...
    <div className="flex min-h-screen bg-[#0a0a0a]">
      ...
      <aside className="w-56 bg-[#0d0d0d] border-r border-[#1a1a1a] flex flex-col fixed h-full z-40 hidden md:flex">
        <div className="p-5 border-b border-[#1a1a1a]">
          ...
          <div className="text-[10px] font-mono-custom text-gray-600 mt-1 uppercase tracking-widest">
            Admin Panel
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={`relative p-1.5 rounded-sm transition-colors ${
                  isActive ? "text-[#C6F135]" : "text-gray-500"
                }`}
              >
                <item.icon size={16} />
